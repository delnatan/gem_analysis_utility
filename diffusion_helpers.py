...
def compute_track_quantities(sdf):
...
def compute_track_stats(sdf):
    ...
    Usage:
    tracks.groupby("particle").apply(
        compute_track_stats).reset_index(level=0)

    """
    mu_x = sdf["x"].mean()
    mu_y = sdf["y"].mean()
    dists_square = (sdf["x"] - mu_x) ** 2 + (sdf["y"] - mu_y) ** 2
    Rg = np.sqrt(np.mean(dists_square))
    npts = len(dists_square)
    prob_mobile = sdf["prob_mobile"].mean()

    return pd.Series(
        {"Rg": Rg, "prob_mobile": prob_mobile, "track_length": npts},
        dtype=object,
    )


def compute_R(illumination_time, frame_interval):
    """
    ...
    R, _ = quad(
        R_integrand, 0, frame_interval, args=(illumination_time, frame_interval)
    )

    return np.round(R / frame_interval, 4)


def compute_msd(sdf, dxy, dt):
